	 ...
	 */
	 #getListValue(key) {
		...
		if (starMatch.length === 1) {
			...
			try {
				let dataFront = eval(`this.#list${(starFront.search(/^\[/giu) === 0) ? "" : "."}${starFront.replace(/\[\\\*\]/giu, "[*]").replace(/\.\\\*/giu, ".*")}`);
				...
				for (let index = 0; index < dataFront.length; index++) {
					...
					let result = eval(`element${(starBack.search(/^\[/giu) === 0) ? "" : "."}${starBack.replace(/\[\\\*\]/giu, "[*]").replace(/\.\\\*/giu, ".*")}`)
