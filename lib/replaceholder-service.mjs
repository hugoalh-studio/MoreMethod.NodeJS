...
class Replaceholder {
	...
	#getListValue(key) {
		...
		if (starMatch.length === 0) {
			try {
				return eval(`let data = ${this.#list}; data${(key.search(/^\[/giu) === 0) ? "" : "."}${key.replace(/\[\\\*\]/giu, "[*]").replace(/\.\\\*/giu, ".*")}`);
			} catch (error) { };
			...
			try {
				...
				if (starFront.length === 0) {
					...
					dataFront = eval(`let data = ${this.#list}; data${(starFront.search(/^\[/giu) === 0) ? "" : "."}${starFront.replace(/\[\\\*\]/giu, "[*]").replace(/\.\\\*/giu, ".*")};`);
				};
				...
				for (let index = 0; index < dataFront.length; index++) {
					...
					let result = eval(`let data = ${JSON.stringify(element)}; data${(starBack.search(/^[\[.]/giu) === 0) ? "" : "."}${starBack.replace(/\[\\\*\]/giu, "[*]").replace(/\.\\\*/giu, ".*")};`);
		...
		if (typeof item === "string") {
			if (item.search(this.#keyREFull) === 0) {
				...
				if (typeof value !== "string" && this.typeTransform === false) {
					value = (Array.isArray(value) === true) ? value.join(", ") : JSON.stringify(value);
				};
				return value;
			} else {
