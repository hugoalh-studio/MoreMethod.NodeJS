...
function isFalse(obj) {
	...
	if (obj === "" || obj === false || obj === null) {
		...
		for (let key in obj) {
			// If there are any keys, then the object is not empty so the object is not false.
...
class Lexer {
	tokenize(stream) {
		...
		while (this._current < stream.length) {
			if (isAlpha(stream[this._current])) {
				...
				let error = new Error(`Unknown character:${stream[this._current]}`);
	...
	_consumeRawStringLiteral(stream) {
		...
		this._current++;
		let literal = stream.slice(start + 1, this._current - 1);
	...
	_consumeNumber(stream) {
		...
		let value = parseInt(stream.slice(start, this._current));
		return {
			type: TOK_NUMBER,
			value: value,
			start: start
...
class Parser {
	parse(expression) {
		...
		if (this._lookahead(0) !== TOK_EOF) {
			...
			let error = new Error(`Unexpected token type: ${t.type}, value: ${t.value}`);
	...
	_match(tokenType) {
		if (this._lookahead(0) === tokenType) {
			...
			let t = this._lookaheadToken(0);
			let error = new Error(`Expected ${tokenType}, got: ${t.type}`);
			error.name = "ParserError";
			throw error;
		}
	}
	...
	_errorToken(token) {
		let error = new Error(`Invalid token (${token.type}): "${token.value}"`);
		error.name = "ParserError";
		throw error;
	}
	...
	_parseSliceExpression() {
		...
		while (currentToken !== TOK_RBRACKET && index < 3) {
			if (currentToken === TOK_COLON) {
				...
				let error = new Error(`Syntax error, unexpected token: ${t.value}(${t.type})`);
				error.name = "Parsererror";
				throw error;
			}
			currentToken = this._lookahead(0);
	...
	_parseProjectionRHS(rbp) {
		...
		if (bindingPower[this._lookahead(0)] < 10) {
			...
			let t = this._lookaheadToken(0);
			let error = new Error(`Sytanx error, unexpected token: ${t.value}(${t.type})`);
			error.name = "ParserError";
			throw error;
		}
		return right;
...
class TreeInterpreter {
	...
	computeSliceParams(arrayLength, sliceParams) {
		...
		if (step === null) {
			...
			let error = new Error("Invalid slice, step cannot be 0");
...
class Runtime {
	...
	_functionReverse(resolvedArgs) {
		let typeName = this._getTypeName(resolvedArgs[0]);
	...
	_functionLength(resolvedArgs) {
		...
		// As far as I can tell, there's no way to get the length of an object without O(n) iteration through the object.
	...
	_functionMax(resolvedArgs) {
		if (resolvedArgs[0].length > 0) {
			let typeName = this._getTypeName(resolvedArgs[0][0]);
	...
	_functionJoin(resolvedArgs) {
		let joinChar = resolvedArgs[0];
		let listJoin = resolvedArgs[1];
	...
	_functionSortBy(resolvedArgs) {
		...
		let requiredType = this._getTypeName(
		...
		let that = this;
		// In order to get a stable sort out of an unstable sort algorithm, we decorate/sort/undecorate (DSU) by creating a new list of [index, element] pairs. In the cmp function, if the evaluated elements are equal, then the index will be used as the tiebreaker. After the decorated list has been sorted, it will be undecorated to extract the original elements.
		let decorated = [];
		for (let i = 0; i < sortedArray.length; i++) {
			decorated.push([i, sortedArray[i]]);
		}
		decorated.sort((a, b) => {
			let exprA = interpreter.visit(exprefNode, a[1]);
			let exprB = interpreter.visit(exprefNode, b[1]);
			if (that._getTypeName(exprA) !== requiredType) {
				throw new TypeError(`expected ${requiredType}, received ${that._getTypeName(exprA)}`);
			} else if (that._getTypeName(exprB) !== requiredType) {
				throw new TypeError(`TypeError: expected ${requiredType}, received ${that._getTypeName(exprB)}`);
			}
			if (exprA > exprB) {
				return 1;
			} else if (exprA < exprB) {
	...
	createKeyFunction(exprefNode, allowedTypes) {
		...
		let interpreter = this._interpreter;
		return (x) => {
			let current = interpreter.visit(exprefNode, x);
			if (allowedTypes.indexOf(that._getTypeName(current)) < 0) {
				throw new TypeError(`expected one of ${allowedTypes}, received ${that._getTypeName(current)}`);
			}
			return current;
		};
	}
...
function search(data, expression) {
	...
	// This needs to be improved. Both the interpreter and runtime depend on each other. The runtime needs the interpreter to support exprefs. There's likely a clean way to avoid the cyclic dependency.
	let runtime = new Runtime();
	let interpreter = new TreeInterpreter(runtime);
