...
function $functionStartsWith(resolvedArgs) {
	return resolvedArgs[0].startsWith(resolvedArgs[1]);
};
function $functionEndsWith(resolvedArgs) {
	return resolvedArgs[0].endsWith(resolvedArgs[1]);
};
function $functionReverse(resolvedArgs) {
...
function $functionAbs(resolvedArgs) {
	return Math.abs(resolvedArgs[0]);
};
function $functionCeil(resolvedArgs) {
	return Math.ceil(resolvedArgs[0]);
};
function $functionAvg(resolvedArgs) {
	let sum = 0;
...
function $functionContains(resolvedArgs) {
	return resolvedArgs[0].indexOf(resolvedArgs[1]) >= 0;
};
function $functionFloor(resolvedArgs) {
	return Math.floor(resolvedArgs[0]);
};
function $functionLength(resolvedArgs) {
	if (!isObject(resolvedArgs[0])) {
		return resolvedArgs[0].length;
	};
	return Object.keys(resolvedArgs[0]).length;
};
function $functionMerge(resolvedArgs) {
	let merged = {};
...
function $functionMax(resolvedArgs) {
...
function $functionMin(resolvedArgs) {
	if (resolvedArgs[0].length > 0) {
		if (getTypeName(resolvedArgs[0][0]) === TYPE_NUMBER) {
...
function $functionSum(resolvedArgs) {
	let sum = 0;
...
function $functionType(resolvedArgs) {
...
function $functionKeys(resolvedArgs) {
	return Object.keys(resolvedArgs[0]);
};
function $functionValues(resolvedArgs) {
	return Object.values(resolvedArgs[0]);
};
function $functionJoin(resolvedArgs) {
	return resolvedArgs[1].join(resolvedArgs[0]);
};
function $functionToArray(resolvedArgs) {
	if (getTypeName(resolvedArgs[0]) === TYPE_ARRAY) {
		return resolvedArgs[0];
	};
	return [resolvedArgs[0]];
};
function $functionToString(resolvedArgs) {
	if (getTypeName(resolvedArgs[0]) === TYPE_STRING) {
		return resolvedArgs[0];
	};
	return JSON.stringify(resolvedArgs[0]);
};
function $functionToNumber(resolvedArgs) {
...
function $functionNotNull(resolvedArgs) {
	for (let i = 0; i < resolvedArgs.length; i++) {
...
function $functionSort(resolvedArgs) {
	return resolvedArgs[0].slice(0).sort();
};
class JMESPath {
	constructor() {
		this.$runtimeFunctionTable = {
			...
			"reverse": {
				_func: $functionReverse,
				_signature: [{ types: [TYPE_STRING, TYPE_ARRAY] }]
			},
			"to_array": {
				_func: $functionToArray,
				_signature: [{ types: [TYPE_ANY] }]
			},
			"to_string": {
				_func: $functionToString,
				_signature: [{ types: [TYPE_ANY] }]
			},
			"to_number": {
				_func: $functionToNumber,
				_signature: [{ types: [TYPE_ANY] }]
			},
			"not_null": {
				_func: $functionNotNull,
	...
	$tokenize(stream) {
		...
		let identifier;
		let token;
		while (this.$lexerCurrent < stream.length) {
			if (isAlpha(stream[this.$lexerCurrent])) {
				start = this.$lexerCurrent;
			...
			} else if (typeof BASICTOKENS[stream[this.$lexerCurrent]] !== "undefined") {
				tokens.push({
					type: BASICTOKENS[stream[this.$lexerCurrent]],
					value: stream[this.$lexerCurrent],
					start: this.$lexerCurrent
				});
				this.$lexerCurrent += 1;
			} else if (isNum(stream[this.$lexerCurrent])) {
				token = this.$consumeNumber(stream);
				tokens.push(token);
			} else if (stream[this.$lexerCurrent] === "[") {
				// No need to increment this.$current. This happens in _consumeLBracket
			...
			} else if (typeof OPERATORSTARTTOKEN[stream[this.$lexerCurrent]] !== "undefined") {
				tokens.push(this.$consumeOperator(stream));
			} else if (typeof SKIPCHARS[stream[this.$lexerCurrent]] !== "undefined") {
				// Ignore whitespace.
				this.$lexerCurrent += 1;
			} else if (stream[this.$lexerCurrent] === "&") {
				start = this.$lexerCurrent;
				this.$lexerCurrent += 1;
				if (stream[this.$lexerCurrent] === "&") {
					this.$lexerCurrent += 1;
					tokens.push({
						type: TOK_AND,
						value: "&&",
				...
				throw new LexerError(`Unknown character:${stream[this.$lexerCurrent]}`);
			};
		};
		return tokens;
	};
	$consumeUnquotedIdentifier(stream) {
		let start = this.$lexerCurrent;
		this.$lexerCurrent += 1;
		while (this.$lexerCurrent < stream.length && isAlphaNum(stream[this.$lexerCurrent])) {
			this.$lexerCurrent += 1;
		};
		return stream.slice(start, this.$lexerCurrent);
	};
	$consumeQuotedIdentifier(stream) {
		let start = this.$lexerCurrent;
		this.$lexerCurrent += 1;
		let maxLength = stream.length;
		while (stream[this.$lexerCurrent] !== "\"" && this.$lexerCurrent < maxLength) {
			// You can escape a double quote and you can escape an escape.
			let current = this.$lexerCurrent;
			if (stream[current] === "\\" && (
				stream[current + 1] === "\\" ||
				stream[current + 1] === "\""
			)) {
			...
			this.$lexerCurrent = current;
		};
		this.$lexerCurrent += 1;
		return JSON.parse(stream.slice(start, this.$lexerCurrent));
	};
	$consumeRawStringLiteral(stream) {
		...
		this.$lexerCurrent += 1;
		let literal = stream.slice(start + 1, this.$lexerCurrent - 1);
	...
	$consumeNumber(stream) {
		...
		this.$lexerCurrent += 1;
		let maxLength = stream.length;
		while (isNum(stream[this.$lexerCurrent]) && this.$lexerCurrent < maxLength) {
			this.$lexerCurrent += 1;
		};
		let value = Number.parseInt(stream.slice(start, this.$lexerCurrent));
		return {
			type: TOK_NUMBER,
			value: value,
			start: start
	...
	$consumeLiteral(stream) {
		...
		let literalString = stream.slice(start, this.$lexerCurrent).trimStart();
	...
	$lookahead(number) {
		return this.$parserTokens[this.$parserIndex + number].type;
	};
	$lookaheadToken(number) {
		return this.$parserTokens[this.$parserIndex + number];
	};
	$advance() {
		this.$parserIndex += 1;
	};
	...
	$led(tokenName, left) {
		...
		switch (tokenName) {
			...
			default:
				errorToken(this.$lookaheadToken(0));
		};
	};
	$match(tokenType) {
	...
	search(data, expression) {
		this.$rootValue = data;
		return this.$visit(this.$parse(expression), data);
	};
	$visit(node, value) {
		let base;
		let collected;
	...
	$callFunction(name, resolvedArgs) {
		let functionEntry = this.$runtimeFunctionTable[name];
	...
	$validateArgs(name, args, signature) {
		...
		if (signature[signature.length - 1].variadic) {
			if (args.length < signature.length) {
				...
				throw new ArgumentError(`${name}() takes at least${signature.length}${pluralized} but received ${args.length}`);
			};
		} else if (args.length !== signature.length) {
			pluralized = signature.length === 1 ? " argument" : " arguments";
			throw new ArgumentError(`${name}() takes ${signature.length}${pluralized} but received ${args.length}`);
		};
		let actualType;
		let currentSpec;
		let typeMatched;
		...
		if (
			...
			if (expected === TYPE_ARRAY) {
				...
				for (let i = 0; i < argValue.length; i++) {
					if (!this.$typeMatches(getTypeName(argValue[i]), subtype, argValue[i])) {
						return false;
					};
				};
				return true;
