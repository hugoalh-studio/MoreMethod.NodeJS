...
const isJSONXChain = require("./internal/is-json-x-chain.js");
/**
 * @private
 ...
 * @returns {any}
 */
function nestifyInternal(item, option) {
	if (Array.isArray(item) === true) {
		return item.map((element) => {
			return nestifyInternal(element);
		});
	};
	if (advancedDetermine.isObjectPair(item) !== false) {
		...
		let queueArrayOnly = [];
		let queueFault = [];
		let queueObjectPairChain = [];
		let queueObjectPairOnly = [];
		let result;
		itemKeys.forEach((key) => {
			if (key.search(/^\[(?:0|[1-9]\d*?)\]$/giu) === 0) {
				queueArrayOnly.push(key);
			} else if (key.search(/^\[(?:0|[1-9]\d*?)\](?:\.[$_a-z][$\d_a-z]*?|\[\d\]|\[[1-9]\d+?\])*?$/giu) === 0) {
				queueArrayChain.push(key);
			} else if (key.search(/^[$_a-z][$\d_a-z]*?$/giu) === 0) {
				queueObjectPairOnly.push(key);
			} else if (key.search(/^[$_a-z][$\d_a-z]*?(?:\.[$_a-z][$\d_a-z]*?|\[\d\]|\[[1-9]\d+?\])*?$/giu) === 0) {
		...
		while (queueFault.length > 0) {
			...
			result[keyCurrent] = ((option.recursively === true) ? nestifyInternal(item[keyCurrent], option) : item[keyCurrent]);
		};
		...
		while (queueObjectPairChain.length > 0) {
			...
			let keyCurrentFront = keyCurrent.match(/^[$_a-z][$\d_a-z]*?(?=[.[])/giu)[0];
			...
			result[keyCurrentFront] = nestifyInternal(delta, option);
		};
		while (queueArrayOnly.length > 0) {
			...
			if (advancedDetermine.isObjectPair(result) === false) {
				...
				result[Number(keyCurrent.replace(/^\[(?<index>0|[1-9]\d*?)\]$/giu, "$<index>"))] = ((option.recursively === true) ? nestifyInternal(item[keyCurrent], option) : item[keyCurrent]);
			} else {
		...
		while (queueArrayChain.length > 0) {
			...
			if (advancedDetermine.isObjectPair(result) === false) {
				...
				let keyCurrentFront = keyCurrent.match(/^\[(?:0|[1-9]\d*?)\](?=\.[$_a-z][$\d_a-z]*?|\[\d\]|\[[1-9]\d+?\])/giu)[0];
				let keyCurrentFrontIndex = Number(keyCurrentFront.replace(/^\[(?<index>0|[1-9]\d*?)\]$/giu, "$<index>"));
...
function nestify(item, option = {}) {
	...
	option.recursively = ((typeof option.recursively === "undefined") ? true : option.recursively);
	if (typeof option.recursively !== "boolean") {
		throw new TypeError(`Argument \`option.recursively\` must be type of boolean!`);
	};
	return nestifyInternal(item, option);
};
