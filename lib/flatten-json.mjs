	...
	if (advancedDetermine.isArray(item) === true) {
		...
		Object.keys(item).forEach((element) => {
			if (element.search(/[-\s*]/giu) !== -1) {
				throw new SyntaxError(`Function \`flattenJSON\` is not supported object key(s) with hyphen, spacing, or star character(s)!`);
			};
			if (option.currentDepth < option.maximumDepth) {
				let data = flattenJSONInternal(
					...
					{
						currentDepth: option.currentDepth + 1,
						currentKey: `${option.currentKey}${(option.currentKey.length === 0) ? "" : "."}${element}`,
						keepArray: option.keepArray,
						maximumDepth: option.maximumDepth
					}
				);
				...
				result[`${option.currentKey}${(option.currentKey.length === 0) ? "" : "."}${element}`] = item[element];
			};
		});
	} else {
