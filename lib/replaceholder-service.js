...
class Replaceholder {
	...
	constructor(list, option = {}) {
		...
		let prefixREEscape = escapeRegularExpressionSpecialCharacter(
			this.prefix,
			{
				forwardSlash: true,
				hyphen: true
			...
			suffixREEscape = escapeRegularExpressionSpecialCharacter(
				this.suffix,
				{
					forwardSlash: true,
					hyphen: true
		...
		let keyMatrix = `${prefixREEscape}.+?${suffixREEscape}`;
	...
	#getListValue(key) {
		...
		if (starMatch.length === 0) {
			try {
				return eval(`let database = ${this.#list}; database${(key.search(/^\[/giu) === 0) ? "" : "."}${key.replace(/\[\\\*\]/giu, "[*]").replace(/\.\\\*/giu, ".*")}`);
			} catch (error) {
				return undefined;
			};
		};
		if (starMatch.length === 1) {
			...
			try {
				let dataFront = eval(`let database = ${this.#list}; database${(starFront.search(/^\[/giu) === 0) ? "" : "."}${starFront.replace(/\[\\\*\]/giu, "[*]").replace(/\.\\\*/giu, ".*")};`);
				...
				for (let index = 0; index < dataFront.length; index++) {
					...
					let result = eval(`let database = ${elementStringify}; database${(starBack.search(/^[\[.]/giu) === 0) ? "" : "."}${starBack.replace(/\[\\\*\]/giu, "[*]").replace(/\.\\\*/giu, ".*")};`);
