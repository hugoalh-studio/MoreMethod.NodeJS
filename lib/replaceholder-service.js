...
class Replaceholder {
	...
	#getListValue(key) {
		...
		if (starMatch.length === 0) {
			let keyChain = key.match(/^[$_a-z][$0-9_a-z]*?(?=[.[])|(?<=\.)[$_a-z][$0-9_a-z]*?(?=[.[])|(?<=\.)[$_a-z][$0-9_a-z]*?$|(?<=\[)([0-9]|[1-9][0-9]+?)(?=\])/giu),
				data = this.#list;
			while (keyChain.length > 0 && typeof data !== "undefined") {
				let keyCurrent = keyChain.shift();
				if (keyCurrent.search(/^\[(?:[0-9]|[1-9][0-9]+?)\]$/giu) === 0) {
					keyCurrent = Number(keyCurrent.replace(/^\[(?<index>[0-9]|[1-9][0-9]+?)\]$/giu, "$<index>"));
				};
			...
			if (starFront.length === 0) {
				...
				let keyChainFront = starFront.match(/^[$_a-z][$0-9_a-z]*?(?=[.[])|(?<=\.)[$_a-z][$0-9_a-z]*?(?=[.[])|(?<=\.)[$_a-z][$0-9_a-z]*?$|(?<=\[)([0-9]|[1-9][0-9]+?)(?=\])/giu);
			...
			for (let index = 0; index < dataFront.length; index++) {
				let element = dataFront[index],
					keyChainBack = starBack.match(/^[$_a-z][$0-9_a-z]*?(?=[.[])|(?<=\.)[$_a-z][$0-9_a-z]*?(?=[.[])|(?<=\.)[$_a-z][$0-9_a-z]*?$|(?<=\[)([0-9]|[1-9][0-9]+?)(?=\])/giu);
